...
two_sum <- function(nums_vector,target){
  ...
  for (i in seq_along(nums_vector)) {
    h_two_sum[(target - nums_vector[i])] <- i
  }
  
  for (j in 1:length(h_two_sum)) {
    complement <- as.character((target - as.numeric(names(h_two_sum)[j])))
    if (has.key(complement, h_two_sum)) {
      answers <- c(answers, list(c(h_two_sum[[names(h_two_sum)[j]]], h_two_sum[[complement]])))
    }
  }
  return(answers)
}
