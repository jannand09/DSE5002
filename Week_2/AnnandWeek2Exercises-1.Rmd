...
difftime(most_recent_order, oldest_order, unit="days")

diff_years <- interval(oldest_order, most_recent_order) %/% years(1)
diff_years

difftime(most_recent_order, oldest_order, unit="weeks")

```

The number of days between the most recent order and the oldest order is 1457 days. The difference in years in 3 years. The difference in weeks is 208 weeks.
...
average_ship <- difftime(sales$Ship.Date, sales$Order.Date, unit="days")
mean(average_ship)

```

The average number of days it takes to ship an order is 3.91 days.
...
```{r}
# Your code here
temp_char <- stringr::str_split_fixed(string=sales_csv$Customer.Name,pattern=' ',n=2)
```
